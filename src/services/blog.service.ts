...
import { BlogData, BlogParams, BlogRespone, UpdateLikeBlogParam } from "@/types/blogType";
...
//like blog
const updateLikeBlog = async ({ id, ipAddress }: UpdateLikeBlogParam): Promise<void> => {
  const res = await HttpService.patch(`/blog/likes/${id}`, { ipAddress });
  return res.data?.data;
};

//views blog
const updateViewBlog = async ({ id, ipAddress }: UpdateLikeBlogParam): Promise<void> => {
