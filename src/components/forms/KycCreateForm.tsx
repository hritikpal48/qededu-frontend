...
export default function KycCreateForm({ onComplete }: KycCreateFormProps) {
  ...
  const onSuccess = (result: any) => {
    ...
    toast.success(
      "✅ KYC request submitted successfully. It will be reviewed shortly."
    );
    onComplete();
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="mt-4">
        {renderUploadBox("Upload PAN Card", panImage, "pan", true)}
      </div>
          ...
          render={({ field }) => (
            <SelectInput
              ...
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) => {
                const stateName = e.target.value;
                field.onChange(stateName); // Save state name
                const selectedState = bankStates.find(
                  (s) => s.name === stateName
                );
                if (selectedState) {
                  const selectedCountry = countries.find(
                    (c) => c.name === watch("kycData.bankDetails.country")
                  );
                  if (selectedCountry) {
                    const citiesList =
